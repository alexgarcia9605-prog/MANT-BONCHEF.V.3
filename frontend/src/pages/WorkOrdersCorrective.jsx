...
import { cn, formatDate, getStatusLabel, getPriorityLabel, getFailureCauseLabel } from '../lib/utils';
import {
    Plus,
    Search,
    ClipboardList,
...
export default function WorkOrdersCorrective() {
    ...
    const fetchData = async () => {
        try {
            const [ordersRes, deptsRes] = await Promise.all([
                axios.get(`${API}/work-orders?type=correctivo`),
    ...
    return (
        <div className="space-y-6 animate-fade-in" data-testid="work-orders-corrective-page">
            <PageHeader
                title="Órdenes Correctivas"
                description="Gestiona las reparaciones y mantenimientos correctivos"
            >
                <Button onClick={() => navigate('/work-orders/corrective/new')} data-testid="new-corrective-order-btn">
                ...
                <CardContent className="p-4">
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1 relative">
                            ...
                            <Input
                                placeholder="Buscar por título, máquina o número de parte..."
                                value={search}
                ...
                <CardContent className="p-0">
                    {filteredOrders.length === 0 ? (
                        <div className="empty-state py-12">
                            ...
                            <p className="text-muted-foreground">No hay órdenes correctivas</p>
                            <Button
                                variant="outline"
                                className="mt-4"
                                onClick={() => navigate('/work-orders/corrective/new')}
                            >
                                Crear primera orden correctiva
                            </Button>
                        </div>
                    ) : (
                                        ...
                                        <tr key={order.id} data-testid={`order-row-${order.id}`}>
                                            ...
                                            <td>
                                                {order.spare_part_used ? (
                                                    <div>
                                                        <p className="text-xs font-medium">{order.spare_part_used}</p>
                                                        {order.spare_part_reference && (
                                                            <p className="text-xs text-muted-foreground mono">{order.spare_part_reference}</p>
                                                        )}
                                                    </div>
                                                ) : '-'}
                                            </td>
